"""The relation type: what a verb is, declared as an sldb document owned by kgdb."""
...
class RelationTypeDoc(StructuredNLDoc):
    ...
    kgdb refuses any edge whose ``relation_type`` has no tracked RelationTypeDoc.
    The structural relations kgdb itself produces are shipped as RelationTypeDocs
    too (see ``kgdb.models.builtin``), so a graph is described entirely by
    documents.
    """
...
""".strip()
    ...
    source_types: list[str] = Field(
        ...
        description="Node classes allowed as source: model names (with inheritance) or kgdb node types; empty means any.",
    )
    target_types: list[str] = Field(
        default_factory=list,
        description="Node classes allowed as target: model names (with inheritance) or kgdb node types; empty means any.",
    )
    condition: str = Field(
        default="",
